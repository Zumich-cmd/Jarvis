...
Створи або відредагуй `appsettings.json` у корені проекту.
...
Важливо:
- `GroqApiKey` потрібен для AI-відповідей і Groq Whisper.
- `ElevenLabsApiKey` має бути Secret Key, який починається з `sk_`.
...
Не публікуй реальні API-ключі в GitHub. Перед завантаженням репозиторію заміни значення в `appsettings.json` на порожні або додай файл у `.gitignore`.

## Примітки

- SQLite база створюється локально для історії, пам'яті й нагадувань.
